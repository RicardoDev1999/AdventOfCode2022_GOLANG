	...
	for y := maxCratesArrSize - 1; y >= 0; y-- {
		...
		for x := 0; x < len(stacks); x++ {
			...
			if len(stack.Crates) > y {
				...
				var crateToMove = sourceStack.Crates[len(sourceStack.Crates)-index]
...
func main() {
	...
	for i := 0; i < len(moves); i++ {
		...
		//Move one crate behaviour
		for i := 0; i < move.Value; i++ {
			var crateToMove = stackToRemove.Crates[len(stackToRemove.Crates)-1]

			//Remove crate from source stack
			stackToRemove.Crates = stackToRemove.Crates[:len(stackToRemove.Crates)-1]

			//Append to destination stack
			var stackToAppend = &stacks[move.Destination-1]
			stackToAppend.Crates = append(stackToAppend.Crates, crateToMove)
		}
	}
}
